 ...
 * It renders objects in the objects List using a few worker threads, creating
 * as many threads as processors available.
 * 
...
public class Camera extends JPanel {
	...
	/**
	 * An array of colors that represents an intermediate storage method for
	 * storing the screen buffer
	 */
	protected Color[][] fbuf;

	/**
	 * The current direction at the time of the start of the render, so as to
	 * avoid visual tears from the user turning during the render
	 */
	protected Vector rdir;

	/**
	 * The current location at the time of the start of the render, so as to
	 * avoid visual tears from the user moving during the render
	 */
	...
	public Camera(Point loc, Vector dir) {
		...
		for (int i = 0; i < numProc; i++) {
			CameraSlave c = new CameraSlave(this, d * i, 0, d * (i + 1),
					SC_HEIGHT);
			c.start();
			slaves.add(c);
		}
	...
	/**
	 * Paints the cached screen to the graphics object and then notifies the
	...
	/**
	 * Calculates what should be drawn to the screen with the current location
	 * and direction
	 */
	...
	/**
	 * iterates through all worker threads and if any are not done, return true
	 * as we must keep waiting
	 * 
	 * @return
	 */
	...
	public void drawRange(int x1, int y1, int x2, int y2) {
		for (int x = x1; x < x2; x += R_INC) {
			for (int y = y1; y < y2; y += R_INC) {
				// get the vector from the camera point to the given pixel on
				// the virtual screen
				Vector v = rdir.add(getVectorForPixel(x + R_INC / 2, y + R_INC
						/ 2, rightU, upU));

				// find the closest object for that given pixel and draw it's
				// color
	...
	protected Vector getVectorForPixel(int x, int y, Vector right, Vector up) {
		int rx = x - SC_WIDTH / 2;
		int ry = SC_HEIGHT / 2 - y; // y is negated because for graphics top is
		// 0, whereas in real life bottom is -
		double px = rx * dx;
		double py = ry * dy;
	...
	protected TColorTransfer closestInFront(Vector dir, Point px) {
		...
		for (ThreeDeeObject p : objects) {
			...
			if (min.t > o.t && o.t >= 0 && o.t == o.t && o.c != null
					&& o.c.getAlpha() != 0) {
				min = o;
			}
		}
	...
	/**
	 * Register a given thread to be sent an interrupt when done painting the
	 * buffer
	 * 
	 * @param t
	 */
