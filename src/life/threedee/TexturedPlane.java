...
/**
 * A plane that has a texture on it, does not extend past it's texture. The
 * origin point given is the bottom left corner of the image
 * 
...
public class TexturedPlane extends Plane {
	...
	/**
	 * Represents right with regards to this plane, must be recalculated if the
	 * plane is rotated
	 */
	public Vector right;

	/**
	 * Represents up with regards to this plane, must be recalculated if the
	 * plane is rotated
	 */
	public Vector up;
	...
	public TexturedPlane(Point p, Vector n, BufferedImage texture) {
		...
		if (dirPolar[0] == dirPolar[0] && dirPolar[1] == dirPolar[1]) {
			up = Vector
					.fromPolarTransform(dirPolar[0], PI / 2 + dirPolar[1], 1);
	...
	/**
	 * Calculates the pixel at a given point of intersection, null if it is
	 * outside the image
	 * 
	 * @param inter
	 * @return
